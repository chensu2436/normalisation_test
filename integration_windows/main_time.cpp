...
Mat getUndistortedImage(Mat, Mat, Mat);
...
int main(int argc, char **argv)
{
    ...
    auto duration_cam_params = duration_cast<microseconds>(stop - start);

    start = high_resolution_clock::now();
    auto img = loadImage(imgFile);
    stop = high_resolution_clock::now();
    auto duration_loading_image = duration_cast<microseconds>(stop - start);

    start = high_resolution_clock::now();
    img = getUndistortedImage(img, cameraParams["camera_matrix"], cameraParams["distortion"]);
    stop = high_resolution_clock::now();
    auto duration_undistorting_image = duration_cast<microseconds>(stop - start);
    
    start = high_resolution_clock::now();
    auto faceModel = loadFaceModel("./face_model.txt");
    stop = high_resolution_clock::now();
    auto duration_loading_face_model = duration_cast<microseconds>(stop - start);
    // cout << "face model = " << endl << " "  << faceModel << endl << endl;
    
    start = high_resolution_clock::now();
    auto landmarks = getLandmarks(img);
    stop = high_resolution_clock::now();
    auto duration_getting_landmarks = duration_cast<microseconds>(stop - start);
    ...
    stop = high_resolution_clock::now();
    auto duration_getting_headpose = duration_cast<microseconds>(stop - start);

    start = high_resolution_clock::now();
    auto res = normalize(img, cameraParams["camera_parameter"], headPose[0], headPose[1], faceModel);
    stop = high_resolution_clock::now();
    auto duration_normalisation = duration_cast<microseconds>(stop - start);

    cout << "loading camera parameters: " << duration_cam_params.count() << "ms" << endl;
    cout << "loading image: " << duration_loading_image.count() << "ms" << endl;
    cout << "undistorting image: " << duration_undistorting_image.count() << "ms" << endl;
    cout << "loading general face model: " << duration_loading_face_model.count() << "ms" << endl;
    cout << "getting landmarks from FEA: " << duration_getting_landmarks.count() << "ms" << endl;
    cout << "getting headpose using cv2: " << duration_getting_headpose.count() << "ms" << endl;
    cout << "normalising eye patch: " << duration_normalisation.count() << "ms" << endl;
...
Mat getUndistortedImage(Mat originalImg, Mat cameraMatrix, Mat cameraDistortion) {
    //cout << "undistoring image" << endl;
    Mat undistorted;
    undistort(originalImg, undistorted, cameraMatrix, cameraDistortion);
